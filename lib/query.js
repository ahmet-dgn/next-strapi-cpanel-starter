...
export async function getSinglePage(slug, locale) {
  const query = await fetchDataDynamicPage(
    "/api/pages?populate=Blocks.Content.Image&populate=Image&populate=Blocks.Image&populate=Blocks.Resim&populate=localizations&populate=Blocks.products.MainImage&populate=Blocks.products.category&populate=SEO&locale=all&filters[Slug][$eq]=",
    slug,
    locale
  );
